...
export default function Footer() {
  ...
  const program = nav.find(
    (n) => !("href" in n) && n.label.includes("프로그램")
  );
  const submission = nav.find(
    (n) => !("href" in n) && n.label.includes("초록")
  );
  const registration = nav.find(
    (n) => !("href" in n) && n.label.includes("참가")
  );
    ...
    <footer className="mt-10 border-t bg-gray-50">
      <div className="mx-auto max-w-6xl px-4 py-4">
        ...
        <div className="mb-8 text-center">
          <p className="whitespace-pre-line text-[13px] leading-6 text-gray-800">
            {societyInfo}
          </p>
        </div>
        ...
        >
          ...
          <div className="w-full max-w-[220px]">
            <h4 className="mb-2 text-[13px] font-semibold text-gray-900">
              학술대회 안내
            </h4>
      ...
      <div className="border-t bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-center px-4 py-6 text-[12px] text-gray-600">
          <p className="text-center uppercase tracking-tight">
            COPYRIGHT KOREAN SOCIETY OF AGRICULTURAL AND FOREST METEOROLOGY ALL
            RIGHTS RESERVED.
          </p>
        </div>
      </div>
